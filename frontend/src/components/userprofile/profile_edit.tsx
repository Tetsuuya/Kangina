...
export const ProfileEditModal: React.FC<ProfileEditModalProps> = ({ 
    ...
    const { updateProfile, updateDietary } = useProfile();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await updateDietary(dietaryPreferences);
            onClose();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[9999]">
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-3 right-3 text-gray-600 hover:text-gray-900 z-[10001]"
                    aria-label="Close modal"
                >
                    ✕
                </button>

                <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-[#32347C] text-left">
                    Edit Profile
                </h2>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {/* Dietary Preferences with improved responsiveness */}
                    <div className="mt-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {dietaryOptions.map((option) => (
                                <label
                                    key={option.key}
                                    className="flex items-center space-x-2 bg-gray-200 px-3 py-1 rounded-full text-sm cursor-pointer mb-2"
                                >
                                    <input
                                        type="checkbox"
                                        name={option.key}
